...
class ConfusionMatrix(object):
    def __init__(self, num_classes,seg,cla):
    ...
    def compute(self):
        ...
        # 计算precision
        pre = torch.diag(h) / h.sum(0)
        return acc_global, acc, iu, pre
    ...
    def __str__(self):
        acc_global, acc, iu, pre = self.compute()
        if self.seg:
            return (
                'seg global correct: {:.1f}\n'
                'seg average row correct: {}\n'
                'seg IoU: {}\n'
                'seg mean IoU: {:.1f}\n').format(
                    acc_global.item() * 100,
            ...
            return (
                'cla global correct: {:.1f}\n'
                'cla average row correct: {}\n'
                'cla precision: {}\n').format(
                    acc_global.item() * 100,
                    ['{:.1f}'.format(i) for i in (acc * 100).tolist()],
                    ['{:.1f}'.format(i) for i in (pre * 100).tolist()])
